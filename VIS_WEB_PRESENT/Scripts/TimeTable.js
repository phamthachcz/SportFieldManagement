...
window.addEventListener('load', function (event) {
    ...
    $("#courtList").change(function () {
        var dID = $(this).val();
        var dayID = document.getElementById("daySelected").innerHTML;
        $.getJSON("../LoadTimebyFacility", { FacilityID: dID, DayInput: dayID },
            function (data) {
                var select = $("#timefreeList");
                select.empty();
...
function renderCalendar() {
    ...
    for (let i = 0; i < totalCalendarDay; i++) {
        ...
        if (i <= startWeekDay) {
            ...
            calendarDays.innerHTML += `<div class='${dayClass}'><a href="/Home/DetailReservation/1?time=${date.getTime()}"> ${day}</a></div>`;
        } else {
